...
- **LUT 強度スライダー**（0-100%）で LUT 効果を細かく調整
...
- **LUT 強度スライダー**: LUT 強度を 0-100% でリアルタイム調整
  - LUT パネルの "Original" 項目の下に配置
  - 強すぎる LUT 効果（ブリーチバイパスなど）を抑えるのに最適
  - スムーズなドラッグ、パフォーマンス向上のため遅延プレビュー
- **HaldCLUT Level 計算の修正**: 1728x1728 LUT のレベル検出を修正（Level 12）

## v1.4.1 の変更点

- **LUT パネルの再設計**: シンプルなタイトル、折りたたみ可能なパネル、統一されたスタイル
- **レンダリングの改善**: パネル折りたたみ時のゴースト/アーティファクトを修正
- **非同期 LUT 選択**: クリック時に即座に選択状態を表示
